...
class SubstitutionCipherTest < Minitest::Test
  ...
  def test_double_shift_encode
    ...
    assert_equal(ciphertext, SimpleCipher.new('iamapandabear').encode(plaintext))
  end
...
class PseudoShiftCipherTest < Minitest::Test
  def setup
    @cipher = SimpleCipher.new('dddddddddd')
  end
